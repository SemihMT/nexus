...
renderer::renderer(const window &window)
    : m_sdlRenderer(SDL_CreateRenderer(window.GetSDLWindow(), nullptr)) {
  if (!m_sdlRenderer) {
    std::cerr << "SDL_CreateRenderer Error: " << SDL_GetError() << std::endl;
  }
}

renderer::~renderer() { SDL_DestroyRenderer(m_sdlRenderer); }

void renderer::Clear() { SDL_RenderClear(m_sdlRenderer); }

void renderer::Present() { SDL_RenderPresent(m_sdlRenderer); }
...
SDL_Renderer *renderer::GetSDLRenderer() const { return m_sdlRenderer; }
} // namespace nxsg
