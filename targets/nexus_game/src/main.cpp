...
void RunServer(uint16_t port)
{
    ...
    SDL_Color bgColor{140, 20, 7, 255};
    ...
    server
        ...
        .OnDisconnect([&](auto conn)
            ...
            std::scoped_lock lock(stateMutex);
            std::erase_if(players, [id](const Player &p){ return p.ID == id; });
            if (players.empty())
                bgColor = {140, 20, 7, 255}; })
        .OnMessage(MyMessageType::Move, [&](auto conn, auto &msg)
                   {
            SDL_FPoint newPos;
...

            {
                ...
                auto it = std::find_if(players.begin(), players.end(),
                    [&](const Player &p){ return p.ID == conn->GetID(); });
                if (it != players.end())
                    it->pos = newPos;
            }
        ...
        .OnMessage(MyMessageType::Reset, [&](auto conn, auto &)
                   {
            std::scoped_lock lock(stateMutex);
            auto it = std::find_if(players.begin(), players.end(),
                [&](const Player &p){ return p.ID == conn->GetID(); });
    ...
    while (running)
    {
        ...
        if (surf)
        {
            ...
            if (tex)
            {
                SDL_FRect tr{10, 10, (float)surf->w, (float)surf->h};
                SDL_RenderTexture(renderer, tex, nullptr, &tr);
                SDL_DestroyTexture(tex);
            }
            SDL_DestroySurface(surf);
...
void RunClient(const std::string &serverAddress, uint16_t serverPort)
{
    ...
    client
        .OnMessage(MyMessageType::EchoPlayer, [&](auto &msg)
                   {
            std::scoped_lock lock(stateMutex);
            msg >> player; })
        .OnMessage(MyMessageType::PlayerJoined, [&](auto &msg)
                   {
            Player newPlayer;
            msg >> newPlayer;
            std::scoped_lock lock(stateMutex);
            otherPlayers.push_back(newPlayer); })
        .OnMessage(MyMessageType::GetPlayers, [&](auto &msg)
                   {
            std::scoped_lock lock(stateMutex);
            otherPlayers.clear();
        ...
        .OnMessage(MyMessageType::MoveBroadcast, [&](auto &msg)
                   {
            uint32_t   id;
            SDL_FPoint pos;
            msg >> pos.y >> pos.x >> id;
            ...
            if (it != otherPlayers.end())
                it->pos = pos; })
        .OnMessage(MyMessageType::MoveAccepted, [&](auto &msg)
                   {
            SDL_FPoint accepted;
            msg >> accepted.y >> accepted.x;
            std::scoped_lock lock(stateMutex);
            player.pos = accepted; })
        .OnMessage(MyMessageType::PlayerDisconnected, [&](auto &msg)
                   {
            uint32_t id;
            msg >> id;
            std::scoped_lock lock(stateMutex);
            std::erase_if(otherPlayers, [id](const Player &p){ return p.ID == id; }); })
